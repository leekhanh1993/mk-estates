...
class EditAdvertisement extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        var allAdvertisements = nextProps.allAdvertisements;
        var idAdvertisement = nextProps.idAdvertisement;
        if (idAdvertisement) {
            var editAD;
            for (let index = 0; index < allAdvertisements.length; index++) {
                const advertisement = allAdvertisements[index];
                if (advertisement._id === idAdvertisement) {
                    editAD = advertisement
                }
            }

            //setState current advertisement immediately
            var newState = {
                ...this.state,
                title: {
                    value: editAD.title,
                ...
                numbedrooms: {
                    value: editAD.numbedrooms,
                    isValid: true,
                    message: ''
                },
                numfloors: {
                    value: editAD.numfloors,
                    isValid: true,
                    message: ''
                },
                direction: {
                    value: editAD.direction,
                    isValid: true,
                    message: ''
                },
                contactInfo: {
                    value: editAD.contactInfo,
                    isValid: true,
                    message: ''
                },
                address: {
                    value: editAD.address,
                    isValid: true,
                    message: ''
                },
                postDate: {
                    value: editAD.postDate,
                    isValid: true,
                    message: ''
                },
                expiredDate: {
                    value: editAD.expiredDate,
                    isValid: true,
                    message: ''
                },
                idUser: editAD.idUser,
                idProject: {
                    value: editAD.idProject,
                    isValid: true,
                    message: ''
                }
            }
    ...
    handleClose() {
        this.props.setCloseEditAdvertisement(false)
    }
    onSubmit() {
        var { title,
            price,
        ...
        this.setState({
            ...
            postDate: {
                value: this.state.postDate.value,
                isValid: true,
                message: ''
            },
            expiredDate: {
        ...
        for (let index = 0; index < allADs.length; index++) {
            ...
            if (ad._id === this.props.idAdvertisement) {
                continue;
            } else {
        ...
        const checkPostDate = (newPostDate) => {
            if (newPostDate === '') {
                this.setState({
                    postDate: {
                        value: this.state.postDate.value,
                        isValid: false,
                        message: 'Post date can not be blanked!!!!'
                    }
                })
            } else {
                return true
        ...
        var resultCheckPostDate = checkPostDate(postDate.value)
        ...
        if (resultCheckTitle
            ...
            var update = {
                ...
                postDate: this.state.postDate.value,
                expiredDate: this.state.expiredDate.value,
                idUser: this.state.idUser,
                idProject: this.state.idProject.value
            }

            this.props.editAD(update, this.props.idAdvertisement)
            alert("Update Advertisement Successful!!!")
            this.props.setCloseEditAdvertisement(false)
        }
    ...
    render() {
        ...
        return (
            <Modal
                backdrop='static'
                show={this.props.showEditAdvertisement}
            ...
            >
                ...
                <ModalBody>
                    <Form horizontal>
                        ...
                        <FormGroup
                            id="postDate"
                            validationState={this.state.postDate.isValid
                                ? null
                                : 'error'}
                        >
                            ...
                            <Col sm={5}>
                                ...
                                />
                                <FormControl.Feedback />
                                {this.state.postDate.isValid
                                    ? ''
                                    : <HelpBlock>{this.state.postDate.message}</HelpBlock>
                                }
                            </Col>
                        </FormGroup>
...
export default connect(mapStateToProps, { getPROs, editAD })(EditAdvertisement);